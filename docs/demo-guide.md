...
- **Fast dev demo**: normal realtime chat, direct-chat deduplication, browser local-only mode, IndexedDB outbox, automatic retry, notifications, and event-flow visibility.
- **Helper-node demo**: central and helper nodes running at the same time, each with its own SQLite event store, with helper events syncing back to central.
- **Laravel central demo**: the same helper and Vue client running against the separate Laravel/PostgreSQL central server.

## Which Demo To Run

| Goal | Run | Open |
|---|---|---|
| Show the original project quickly | `npm run dev` | `http://localhost:1234` |
| Show helper sync against Node central | `npm run build`, then central/helper `npm run start` commands below | `http://localhost:3001` |
| Show Laravel/PostgreSQL integration | Laravel `php artisan serve`, then `npm run dev:laravel` | `http://localhost:1234?api=http://localhost:3001` |
...
The demo-local-only flag is kept in `sessionStorage`, so this tab stays disconnected after refresh until **Reconnect this tab** is clicked.

If Anna sees a Denis message before Denis reconnects, that should be because the hero panel reports a real peer send, for example `Peer fallback: sent to 1 peer`. Cross-user demo windows do not receive local-only messages through the same-browser local event bus.
...
> Socket.IO is the live transport, but the browser stores user intent first. If the connection drops or the tab is closed, the user action can still be restored and retried later.
...
| Visible behaviour | Technical activity |
|---|---|
| JSON recovery file downloads | IndexedDB events are serialized with a versioned format and checksum. |
| Import rejects wrong files | Client validates recovery dump format and checksum. |
| Imported events deduplicate | Central stores each event once by `eventId`. |

## Helper-Node Demo With Original Node Central
...
| Central later receives events | Helper signs and pushes unsynced events to central. |
...
## Laravel Central Demo

Use this when showing the separate Laravel/PostgreSQL central server.

Terminal 1, in the Laravel repository:
...
1. Use the app through the helper, not directly against Laravel.
2. Create a direct chat and send messages.
3. Stop Laravel while keeping the helper and browser open.
4. Send more messages through the helper.
5. Restart Laravel.
6. Watch helper sync retry and confirm the messages appear in Laravel-backed central history.

What to point out:

| Visible behaviour | Technical activity |
|---|---|
| Client still uses Socket.IO | Browser talks to the Node helper. |
| Helper continues during central outage | Helper writes events to local SQLite. |
| Laravel receives events after recovery | Helper signs pending batches and syncs them. |
| Duplicate events are not projected twice | Laravel deduplicates by `eventId`. |
| Direct chat ids converge | Duplicate direct chats from several helpers remap to the authoritative central chat id. |

## Peer Fallback

After the working demo, explain the WebRTC layer and open [webrtc-peer-mode.md](webrtc-peer-mode.md) for protocol details.
...
> The current project implements peer-assisted recovery, not magic LAN discovery. Browsers need central or helper signalling first. Once a WebRTC data channel exists, local-only tabs can exchange new events, compare event summaries, backfill missing events, and later reconcile with central.
...
The last line means the message is still safe in IndexedDB, but it will not be visible to the other user until reconnect/helper sync/recovery import.
